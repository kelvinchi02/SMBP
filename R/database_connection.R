...
load_supabase_table <- function(table_name) {
  ...
  csv_file <- file.path("data", paste0(table_name, ".csv"))
  # Handle special case for schedule table mapping
  if (table_name == "bus_schedule") csv_file <- "data/schedule.csv"
...
get_latest_timestamp <- function(table_name) {
  ...
  if (file.exists("data/SmartTransit_Integrated.csv")) {
    dt <- fread("data/SmartTransit_Integrated.csv", select = "datetime")
    if (nrow(dt) > 0) {
